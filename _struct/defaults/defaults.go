...
var (
	errInvalidType = errors.New("set defaults -> not a struct pointer")
)
...
func setField(field reflect.Value, defaultVal string) error {
	...
	// If it's initial value... then set the default one!
	if isInitialValue(field) {
		switch field.Kind() {
		case reflect.Bool:
			// Initial value for boolean is False... but false doesn't mean only initial, it means a real value
			// that should be left as it is!
			// That's why, in case if the default value is true, and the real value has being set to false, it should
			// remain false!
			// And initially when generating the field, it should take the default value
			// We have corrected, and the user should use as default values: yes or no for better parsing!
			val := conv.ParseBool(defaultVal)
...
func isInitialValue(field reflect.Value) bool {
	// For boolean type... we should make an exception... false is an initial value, but most of the time we need it
	// to be as it is!
